...
def PupilTrackingAlg(frame, PupilParam, SYSPARAMS, display, ax):
    ...
    if PupilParam.tracking:
        ...
        ) = track_pupil_extquarter_reflection(frame, 0)
    else:
    ...
    PupilParam.center_x = np.mean(
        [PupilParam.x1, PupilParam.x2]
    )  # if tracking is not engaged it will be -1
    ...
    recording = [
        PupilParam.x1 / PupilParam.pixel_calibration,
...
if PupilParam.Sync==1 & etime(clock,[2000 1 1 0 0 0]) - SYSPARAMS.pupil_duration < 0 cmp"""
    ...
    if PupilParam.Ltotaloffx >= (
        DEpupil_tracking_H_OF_BUFFER - 1
    ):  # filter through buffer 5 times
...
def etime(end, start):
    ...
    dt_end = datetime(
        end[0], end[1], end[2], end[3], end[4], int(end[5]), int((end[5] % 1) * 1e6)
    )
...
def handle_boundary_error(PupilParam, frame_size):
    ...
    if PupilParam.BEflag:
        ...
        PupilParam.r1 = [
            [half_frame_width] * frame_size[0],
            range(1, frame_size[0] + 1),
        ]
        ...
        vertical_markers_ydata = np.concatenate(
            [
                np.arange(
                    half_frame_height - calibration_distance,
                    -calibration_distance,
                    -calibration_distance,
                ),
                np.arange(
                    half_frame_height + calibration_distance,
                    frame_size[0] + calibration_distance,
                    calibration_distance,
                ),
            ]
        )
        PupilParam.r2 = [
            [half_frame_width] * len(vertical_markers_ydata),
            vertical_markers_ydata,
        ]

        # Set horizontal centerline visualization
        PupilParam.r3 = [
            range(1, frame_size[1] + 1),
            [half_frame_height] * frame_size[1],
        ]
        ...
        horizontal_markers_xdata = np.concatenate(
            [
                np.arange(
                    half_frame_width - calibration_distance,
                    -calibration_distance,
                    -calibration_distance,
                ),
                np.arange(
                    half_frame_width + calibration_distance,
                    frame_size[1] + calibration_distance,
                    calibration_distance,
                ),
            ]
        )
        PupilParam.r4 = [
            horizontal_markers_xdata,
            [half_frame_height] * len(horizontal_markers_xdata),
        ]
...
def handle_TCA_computation(PupilParam, StimParams, SYSPARAMS):
    ...
    if PupilParam.track_error >= -1:
        ...
        SYSPARAMS.pupil_diff_x[PupilParam.Ltotaloffx - 1] = (
            difx / PupilParam.pixel_calibration
        )
        SYSPARAMS.pupil_diff_y[PupilParam.Ltotaloffx - 1] = (
            dify / PupilParam.pixel_calibration
        )
        # Handle message display based on FPS
        message = TCA_message(
            SYSPARAMS.PupilCamerafps,
            difx,
            dify,
            SYSPARAMS.pupil_TCA_x,
            SYSPARAMS.pupil_TCA_y,
            PupilParam.pixel_calibration,
        )
        print(message)
    ...
    if PupilParam.TCA_comp:
        ...
        if (abs(x_offset) > 0 or abs(y_offset) > 0) and PupilParam.show_reference:
            ...
            if SYSPARAMS.real_system == 1:
                aligncommand = (
                    f"UpdateOffset#{PupilParam.totaloffx}#{PupilParam.totaloffy}#"
...
def TCA_message(current_fps, difx, dify, pupil_TCA_x, pupil_TCA_y, pixel_calibration):
    ...
    if current_fps < 10:
        message = (
            f"Hz= {current_fps}fps mm({difx / pixel_calibration:.1f}, "
...
def handle_video_saving(PupilParam):
    ...
    if (
        PupilParam.saving_video
        and PupilParam.frame_count < PupilParam.MAX_NUM_OF_SAVABLE_FRAMES
    ):
        current_time = datetime.now()
        ...
        if (
            ...
            if PupilParam.pupil_tracking_flag == True:
                ...
                DateString = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())
                PupilData = {
                    "Data": PupilParam.pupil_tracking_Data,
                    "Pixel_calibration": PupilParam.pixel_calibration,
                }
...
def handle_focus_measure(PupilParam, frame, frame_size):
    ...
    if PupilParam.show_focus:
        ...
        roi = frame[
            round(frame_size[0] / 2) - 30 : round(frame_size[0] / 2) + 30,
            0 : frame_size[1],
        ]
        ...
        FM = cv2.filter2D(
            np.float64(gray_roi), -1, laplacian, borderType=cv2.BORDER_REPLICATE
        )
...
def track_pupil_extquarter_reflection(image, is_graph):
    ...
    _, thresh = cv2.threshold(gray_image, 210, 255, cv2.THRESH_BINARY)
